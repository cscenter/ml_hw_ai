...
После создания агента нужно будет сделать сабмит. Однако система сабмитов пока в разработке.
...
Сделайте так, чтобы метод `get_my_name` возвращял строкой ваше имя и фамилию. Метод `offer_action`
возвращял размер предложения для другого агента, а `deal_action` результат согласия или не согласия.
...
4. Пока у нас нет полной автоматизации, поэтому чтобы мы смогли скачать и воспроизвести решения, 
вам нужно будет передать названия ваших `<dockerhub_login>` Алексею Пшеничному и по всем вопросом связанным
с этим заданием можно обрашаться к нему. slack:@izhleba  email: izhleba@gmail.com 

5. Мы оповестим вас о спосообе предоставленяи результатов тестов агентов и наладим процесс 
автоматического переодического их обновления. Следите за новостями.
